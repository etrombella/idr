...
public class PopulateTableRealTime {

	private static Logger logger = LoggerFactory.getLogger(PopulateTableRealTime.class);

	private Properties properties;

	private String pathFile;

	public PopulateTableRealTime(String pathFile) throws IOException {
	...
	public void executeSqlScript(String fileName) throws Exception {
		...
		try (Connection conn = getConnection();) {
			...
			try (Reader reader = new BufferedReader(new FileReader(file))) {
				try (FileWriter logFile = new FileWriter("T:\\OPPER\\jar\\logs\\appOpperScriptRunnerRealTime_"+file.getName()+".log");
	...
	private void readPropertiesFile() throws IOException {
		...
		try (InputStream input = new FileInputStream(pathFile.concat("configRealTime.properties"))) {
