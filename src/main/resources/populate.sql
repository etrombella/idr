...
(
    SELECT
        ...
        opper.dbo.IDIR_ARTICOLI.PRECODICE,
        opper.dbo.IDIR_ARTICOLI.CODICE,
        COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 2))) AS PrelevatoCodice,
        ROW_NUMBER() OVER (ORDER BY  COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 2))) DESC) AS [Rank],
        COUNT( COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 2)))) OVER () AS PrelevatoTot,
        CAST(
            CAST( COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8))) AS NUMERIC(18, 8)) /
            CAST(COUNT( COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8)))) OVER () AS NUMERIC(18, 8)) 
            AS NUMERIC(18, 8)
        ) AS [% su Tot],
        COUNT( COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8)))) OVER (ORDER BY  COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8))) DESC Rows BETWEEN unbounded preceding AND CURRENT row) [PrelevatoCumulato],
        CAST(COUNT( COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8)))) OVER (ORDER BY  COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8))) DESC Rows BETWEEN unbounded preceding AND CURRENT row) AS numeric(18,8))/ CAST(COUNT( COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8)))) OVER () AS numeric(18,8)) [% Cumulata],
        CASE
            WHEN  CAST(COUNT( COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8)))) OVER (ORDER BY  COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8))) DESC Rows BETWEEN unbounded preceding AND CURRENT row) AS numeric(18,8))/ CAST(COUNT( COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8)))) OVER () AS numeric(18,8))<= 0.60 THEN 'A'
            WHEN  CAST(COUNT( COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8)))) OVER (ORDER BY  COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8))) DESC Rows BETWEEN unbounded preceding AND CURRENT row) AS numeric(18,8))/ CAST(COUNT( COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8)))) OVER () AS numeric(18,8))<= 0.85 THEN 'B'
            WHEN  CAST(COUNT( COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8)))) OVER (ORDER BY  COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8))) DESC Rows BETWEEN unbounded preceding AND CURRENT row) AS numeric(18,8))/ CAST(COUNT( COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8)))) OVER () AS numeric(18,8))<= 0.95 THEN 'C'
            ELSE 'D'
        END AS ABC
    FROM
...
(
		...
		opper.dbo.IDIR_ARTICOLI.SOTTOCLASSE_DESCRIZIONE,
         COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 2))) AS PrelevatoSottoClasse,
        ROW_NUMBER() OVER (ORDER BY  COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 2))) DESC) AS [Rank],
        COUNT( COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 2)))) OVER () AS PrelevatoTot,
        CAST(
            CAST( COUNT(CAST([FACT_PRELIEVI_ID] AS DECIMAL(18, 8))) AS NUMERIC(18, 8)) /
